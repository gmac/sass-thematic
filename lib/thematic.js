...
*/
var FIELD_WRAPPER = '____';
var FIELD_PATTERN = FIELD_WRAPPER +'(.+?)'+ FIELD_WRAPPER;
var FIELD_REGEX = new RegExp(FIELD_PATTERN);
var FIELD_REGEX_ALL = new RegExp(FIELD_PATTERN, 'g');
...
SassThematic.prototype = {
  ...
  _parseNode: function(node, parent) {
    ...
    // Keep variable declarations and overrides:
      ...
      if (this._template && isThemeVar && isRefContext) {
        ...
        node.type = id.type;
        node.content = FIELD_WRAPPER + id.content + FIELD_WRAPPER;
      }
  ...
  */
  isValidFieldName: function(field) {
    var match = field.match(FIELD_REGEX);
    return this._template && match && this.vars.hasOwnProperty(match[1]);
  },
  ...
  templateSassOptions: function() {
    return {
      ...
      data: [
        ...
        '@function sass-thematic-var($n){ @return '+FIELD_WRAPPER+'#{unquote($n)}'+FIELD_WRAPPER+'; }',
        this.varsData,
        this.toString()
      ].join('\n')
    };
  ...
  */
  ...
  /**
  * Replaces parsed variable names into field literals.
  ...
  */
  varsToFieldLiterals: function(sass) {
    return sass.replace(/\$([\w-]+)([^:;]*;)/g, function(match, field, terminus) {
      return FIELD_WRAPPER + field + FIELD_WRAPPER + terminus;
    });
  },
  ...
  */
  fieldLiteralsToInterpolations: function(css) {
    var self = this;
    return css.replace(FIELD_REGEX_ALL, function(match, fieldName) {
