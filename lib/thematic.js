 ...
 */
var reducerOptions = ['varsFile', 'varsData', 'themeFile', 'themeData', 'templateOpen', 'templateClose', 'templateSnakeCase', 'outputStyle'];
...
function SassThematic(ast, opts) {
  ...
  if (this.varsFile && !this.varsData) {
    this.varsFile = path.resolve(this.cwd, this.varsFile);
    this.varsData = fs.readFileSync(this.varsFile, 'utf-8');
  } else if (!this.varsData) {
    throw 'No variables file specified.';
  }

  // Write all theme variables into the reducer vars cache:
  var pattern = /(\$[^\s:]+)[\s:]/g;
  var match = pattern.exec(this.varsData);

  while (match) {
    this.vars[ match[1] ] = 1;
    match = pattern.exec(this.varsData);
  }
}
...
SassThematic.prototype = {
  ...
  */
  ...
  reduceNode: function(node, parent) {
    ...
    // Keep variable declarations and overrides:
      ...
      var overrideVar = this.vars.hasOwnProperty(Node.toString(node));
      // Test if the variable is keepable (includes local derivatives)
      var keepableVar = overrideVar || this.locals.hasOwnProperty(Node.toString(node));
    ...
    if (!keep && this.isRemovable(node.type) && this._pruneTree) {
  ...
  * - pruneTree: true for destructive pruning.
  * - template: true for template parsing.
  * @returns {SassThematic} self reference.
  */
  parse: function(opts) {
    var opts = opts || {};
    this._template = Boolean(opts.template || false);
    this._pruneTree = Boolean(opts.pruneTree || false);
    this._pruneVars = Boolean(opts.pruneVars || false);
    this.reduceNode(this.ast);
    return this;
  },

  /**
  * Convenience method for pruning the tree.
  * Deprecated. Use `parse({prune: true})` instead.
  */
  prune: function(opts) {
    var opts = opts || {};
    opts.pruneTree = true;
  ...
  */
  renderTemplate: function(done) {
    ...
    function formatResult(result) {
      return result.css.toString().replace(/____(.+?)____/g, function(match, fieldName) {
