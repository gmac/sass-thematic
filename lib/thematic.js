...
function SassThematic(ast, opts) {
  ...
  this.loadThemeVars();
  this.mapSassVars(this.varsData);
}
...
SassThematic.prototype = {
  ...
  */
  loadThemeData: function() {
    if (!this.themeData) {
      ...
      this.mapSassVars(this.themeData);
    }
  },
  ...
  parseNode: function(node, parent) {
    ...
    if (Array.isArray(node.content)) {
      ...
      for (var i=0; i < node.content.length; i++) {
        keep = this.parseNode(node.content[i], node) || keep;
      }
    }
  ...
  * - pruneTree: true for destructive tree pruning.
  * - pruneVars: true for destructive variables removal.
  ...
  */
  parse: function(opts) {
    ...
    this._pruneTree = Boolean(opts.pruneTree !== undefined ? opts.pruneTree : true);
    this._pruneVars = Boolean(opts.pruneVars !== undefined ? opts.pruneVars : true);
    this.parseNode(this.ast);
    return this;
  },
