  ...
  */
  ...
  filterNode: function(node, parent) {
    ...
    // Keep variable declarations and overrides:
      ...
      // or collected local variables (that may be derrived from overrides)
      var keepVar = this.vars.hasOwnProperty(node.toString()) ||
                    this.locals.hasOwnProperty(node.toString());

      // Keep if the variable is a property definition ($banana: yellow;),
      // of if the variable is overrideable within a value, interpolation, or loop.
      return  (parent.type === NodeType.PROPERTY) || 
              (parent.type === NodeType.VALUE && keepVar) ||
              (parent.type === NodeType.INTERPOLATION && keepVar) ||
              (parent.type === NodeType.LOOP && keepVar);
    }
