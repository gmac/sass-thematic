...
SassThematicPlugin.prototype.validateCache = function(timestamps) {
  ...
  for (var filepath in AST.cache) {
    ...
    if (!cachedFile) reload[filepath] = 1;
  }
...
SassThematicPlugin.prototype.loadCache = function(done) {
  ...
  for (var filename in AST.cache) {
    if (AST.cache.hasOwnProperty(filename) && !AST.cache[filename]) {
...
SassThematicPlugin.prototype.addDependencies = function() {
  ...
  // Loop through all cached files:
  for (var filepath in AST.cache) {
    if (AST.cache.hasOwnProperty(filepath) && AST.cache[filepath] && !currentDeps[filepath]) {
...
SassThematicPlugin.prototype.build = function(opts, done) {
  ...
  opts.includeFiles.forEach(function(filename) {
    if (AST.cache[filename]) {
      if (!file.themeSass || !file.templateSass) {
        var reducer = new SassThematic(file.ast, opts);
        file.themeSass = reducer.prune().ast.toString();
        file.templateSass = reducer.prune({template: true}).ast.toString();
      }

      cssRenderer.sass += AST.cache[filename].themeSass || '';
      templateRenderer.sass += AST.cache[filename].templateSass || '';
    } else {
