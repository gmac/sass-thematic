...
function Importer(opts) {
  ...
  this.data = opts.data;
  this.importer = (typeof opts.importer === 'function') ? opts.importer : null;
...
Importer.prototype = _.extend({}, EventEmitter.prototype, {
  ...
  */
  run: function(done) {
    ...
    function finish(err, file) {
      ...
      else done(null, self._result(file));
      self.emit('end');
    }

    if (this.data) {
      this.createFile(this.file, this.uri, this.file, this.data).parse(finish);
    } else {
  ...
  */
  ...
  /**
  * Formats the result of the file importer.
  * @param {File} file data to return.
  * @returns {Object} standard return format.
  */
  _result: function(file) {
    return {
      ast: file.ast,
      file: file.file,
      includedFiles: Object.keys(this.includedFiles)
    };
  },
  ...
  resolve: function(uri, prev, done) {
    ...
    function lookup(index) {
      ...
      if (!filepath) {
        return done(self.pathLookupError(paths, uri, prevUri));
      }

      fs.readFile(filepath, 'utf-8', function(err, data) {
        if (err && err.code === 'ENOENT') return lookup(index+1);
        else if (err) return done(err);
        done(null, self.createFile(uri, prev, filepath, data));
      });
    }
  ...
  createFile: function(uri, prev, filepath, data) {
    var isFileImport = (prev instanceof File);
    var graph = isFileImport ? prev.graph : [];
    ...
    // Include file imports in table of included files:
    if (isFileImport && !this.includedFiles[filepath]) {
      this.includedFiles[filepath] = 1;
      this.importer && this.importer(filepath);
    }
...
File.prototype = {
  ...
  isParsed: function() {
    return this.ast && self.pendingImports === self.resolvedImports;
  },
  ...
  parse: function(done) {
    ...
    function finish(err) {
      // Only finish when we have an error,
      // or when import parsing is complete.
      // Resolve the full queue of pending callbacks.
    ...
    if (isAsync) setImmediate(finish);
    return this;
  }
};
