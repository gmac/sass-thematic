...
Importer.prototype = _.extend({}, EventEmitter.prototype, {
  ...
  run: function(callback) {
    ...
    if (this.data) {
      new File(this.file, this.data, this).parse().then(done);
    } else {
  ...
  resolve: function(uri, prev, done) {
    var self = this;
    var prevUri = (prev instanceof File) ? prev.file : prev;
    ...
    var lookup = function(index) {
      ...
      if (!filepath) {
        self.emit('error', self._pathLookupError(paths, uri, prevUri));
      }
      ...
      fs.readFile(filepath, 'utf-8', function(err, data) {
        ...
        done(err, self.cacheFile(filepath, data, prev));
      });
    };
  ...
  resolveSync: function(uri, prev) {
    ...
    for (var i=0; i < paths.length; i++) {
      ...
      try {
        ...
        return this.cacheFile(filepath, data, prev);
      } catch (err) {
