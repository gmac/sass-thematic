...
var reducerOptions = ['templateOutputStyle', 'templateOpen', 'templateClose', 'varsFile'];

function Reducer(ast, opts) {
  this.ast = ast;
  this.selector = [];
  this.vars = {};
  this.locals = {};
...
Reducer.prototype = {
  ...
  */
  ...
  // Invokes pruning on the reducer's AST:
  prune: function() {
    ...
    var scss = sass.render({
      ...
      function(err, result) {
        ...
        var css = result.css.toString().replace(/____(.+?)____/g, function(match, $1) {
          return [self.templateOpen, ' ', $1, ' ', self.templateClose].join('');
        });
